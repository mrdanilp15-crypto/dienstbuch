...
def init_db():
    try:
        ...
        migrations = [
            ...
            ("vehicles", "next_oil_change_km", "INT DEFAULT 10000"), ("vehicles", "license_plate", "VARCHAR(50) DEFAULT ''"), ("vehicles", "vehicle_type", "VARCHAR(100) DEFAULT ''"),
            ("vehicle_log", "fuel_liters", "FLOAT DEFAULT 0.0"), 
            ("inventory", "qr_code_id", "VARCHAR(100) DEFAULT ''"), ("inventory", "last_check", "DATE NULL"), ("inventory", "next_check", "DATE NULL"),
            ("inventory", "category", "VARCHAR(100) DEFAULT ''"), ("inventory", "manufacturer", "VARCHAR(100) DEFAULT ''"), ("inventory", "serial_number", "VARCHAR(100) DEFAULT ''")
        ]
...
async def api_login(r: Request, res: Response):
    ...
    if not u or not verify_password(u['password_hash'], d.get('password','')):
        cur.close(); c.close(); return Response(status_code=401)
...
def api_me(r: Request):
    ...
    res = cur.fetchone(); cur.close(); c.close()
    return res
...
def get_weather(r: Request):
    ...
    except: return {"station": s.get("station_name", "Wache"), "temperature": "N/A", "wind": "N/A", "warning_text": "Wetter-API blockiert."}
...
async def save_user(r: Request):
    ...
    if u_id:
        if pw.strip(): cur.execute("UPDATE users SET role=%s, personnel_id=%s, password_hash=%s WHERE id=%s", (role, p_id, hash_password(pw), u_id))
        else: cur.execute("UPDATE users SET role=%s, personnel_id=%s WHERE id=%s", (role, p_id, u_id))
    else:
...
async def save_vehicle(r: Request):
    ...
    if v_id: cur.execute("UPDATE vehicles SET name=%s, radio_name=%s, status=%s, milage=%s, tuv_date=%s, sp_date=%s, next_oil_change_km=%s, license_plate=%s, vehicle_type=%s WHERE id=%s", params + (v_id,))
    else: cur.execute("INSERT INTO vehicles (name, radio_name, status, milage, tuv_date, sp_date, next_oil_change_km, license_plate, vehicle_type) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)", params)
...
async def save_inv(r: Request):
    ...
    if i_id: cur.execute("UPDATE inventory SET item_name=%s, amount=%s, min_amount=%s, unit=%s, location=%s, qr_code_id=%s, last_check=%s, next_check=%s, category=%s, manufacturer=%s, serial_number=%s WHERE id=%s", params + (i_id,))
    else: cur.execute("INSERT INTO inventory (item_name, amount, min_amount, unit, location, qr_code_id, last_check, next_check, category, manufacturer, serial_number) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", params)
...
async def create_ticket(r: Request):
    d = await r.json()
    c = get_db_connection(); cur = c.cursor()
    v_id = parse_val(d.get('vehicle_id')); if str(v_id) == "0": v_id = None
    i_id = parse_val(d.get('inventory_id')); if str(i_id) == "0": i_id = None
...
async def inbound_webhook(req: Request):
    ...
    text = payload.get("text") or payload.get("message") or "Keine Details."
...
def get_attendance(group_id: int, r: Request, session_id: Optional[int] = None):
    """Liest die Personalliste für den Editor."""
    c = get_db_connection()
    cur = c.cursor(dictionary=True)
...
async def save_attendance(r: Request):
    """Speichert den Bericht aus dem Editor."""
    d = await r.json()
    c = get_db_connection(); cur = c.cursor()
...
def get_eri_card(un_number: str, r: Request):
    ...
    res = cur.fetchone(); cur.close(); c.close()
    if res: return res
    return {"un_number": un_clean, "danger_text": "Gefahr unbekannt.", "safety_measures": "GAMS-Regel anwenden.", "first_aid": "Eigenschutz beachten."}
...
async def add_hydrant(r: Request):
    d = await r.json()
    c = get_db_connection(); cur = c.cursor()
    cur.execute("INSERT INTO hydranten (lat, lon, hydrant_type, diameter, last_check) VALUES (%s,%s,%s,%s,%s)", (d.get('lat'), d.get('lon'), d.get('hydrant_type'), d.get('diameter'), parse_val(d.get('last_check'))))
    c.commit(); cur.close(); c.close()
    return {"status": "success"}
...
async def save_event(r: Request):
    ...
    c = get_db_connection(); cur = c.cursor()
    e_id = d.get('id')
    if e_id: cur.execute("UPDATE events SET date=%s, title=%s, responsible=%s WHERE id=%s", (parse_val(d.get('date')), d.get('title'), d.get('responsible'), e_id))
    else: cur.execute("INSERT INTO events (date, title, responsible) VALUES (%s,%s,%s)", (parse_val(d.get('date')), d.get('title'), d.get('responsible')))
    c.commit(); cur.close(); c.close()
    return {"status": "success"}
