...
def api_me(r: Request):
    """Liefert die persönlichen Daten (Profil, Qualifikationen) des eingeloggten Nutzers."""
...
def get_weather(r: Request):
    """Ruft Live-Wetterdaten für die in den Einstellungen definierte Dienststelle ab."""
    c = get_db_connection()
    cur = c.cursor(dictionary=True)
    cur.execute("SELECT setting_key, setting_value FROM settings")
...
async def inbound_webhook(req: Request):
    """Verarbeitet Alarme von aPager, Divera, etc."""
    try: 
    ...
    # Outbound Pushes (Divera & Alamos) triggern
...
# ==========================================
# 12. DIENSTPLAN (EVENTS) & BERICHTE (SESSIONS)
# ==========================================
@app.get("/api/events")
