...
class VehicleCreateDto(BaseModel): name: str; radio_name: str; status: int; milage: int; tuv_date: Optional[str] = None; sp_date: Optional[str] = None
...
def init_database():
    ...
    # Standard-Kameraden anlegen, damit "Mein Bereich" nicht leer ist
    cur.execute("INSERT IGNORE INTO personnel (id, name, rank, is_agt, is_gf) VALUES (1, 'System Administrator', 'Brandmeister', 1, 1)")
    
    # Admin Account verknüpfen
    cur.execute("SELECT COUNT(*) FROM users WHERE username = 'admin'")
    if cur.fetchone()[0] == 0:
        cur.execute("INSERT INTO users (username, password_hash, role, is_first_login, personnel_id) VALUES (%s, %s, %s, 0, 1)", ("admin", hash_password("admin123"), "admin"))
    
    # Dummy Fahrzeuge
    cur.execute("SELECT COUNT(*) FROM vehicles")
    if cur.fetchone()[0] == 0:
        cur.execute("INSERT INTO vehicles (name, radio_name, status) VALUES ('HLF 20', 'Florian 40/1', 2), ('LF 10', 'Florian 43/1', 2)")
...
def api_auth_me(request: Request):
    ...
    conn = get_db_connection(); cur = conn.cursor(dictionary=True)
    cur.execute("""
        SELECT u.role, p.name as personnel_name, p.is_agt, p.is_maschinist, p.is_gf, DATE_FORMAT(p.g26_3_date, '%d.%m.%Y') as g26 
        FROM users u LEFT JOIN personnel p ON u.personnel_id = p.id WHERE u.username = %s
    """, (user["username"],))
...
def get_events():
    conn = get_db_connection(); cur = conn.cursor(dictionary=True)
    cur.execute("SELECT id, DATE_FORMAT(date, '%d.%m.%Y') as date_formatted, title, responsible FROM events ORDER BY date ASC")
    res = cur.fetchall(); cur.close(); conn.close(); return res
...
def get_att(group_id: int, request: Request, session_id: Optional[int] = None):
    ...
    # REPARATUR: Echter LEFT JOIN, damit das Kreuzchen geladen wird!
    query = """
