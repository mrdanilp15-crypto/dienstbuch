...
# --- HILFSFUNKTIONEN ---
def parse_val(v):
    """Filtert leere Strings zu NULL, verhindert Datenbank-Crashes"""
...
async def save_inv(r: Request):
    ...
    if not qr_id or len(str(qr_id).strip()) == 0: qr_id = f"FW-QR-{secrets.token_hex(4).upper()}"
    i_id = d.get('id')
