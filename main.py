...
async def inbound_webhook(req: Request):
    ...
    if st.get('divera_webhook'):
        try:
            urllib.request.urlopen(urllib.request.Request(st['divera_webhook'], method="POST", data=outbound, headers=headers), timeout=2)
        except: pass
    if st.get('alamos_fe2_url'):
        try:
            urllib.request.urlopen(urllib.request.Request(st['alamos_fe2_url'], method="POST", data=outbound, headers=headers), timeout=2)
        except: pass
...
async def save_event(r: Request):
    ...
    e_id = d.get('id')
    if e_id:
        cur.execute("UPDATE events SET date=%s, title=%s, responsible=%s WHERE id=%s", (parse_val(d.get('date')), d.get('title'), d.get('responsible'), e_id))
    else:
        cur.execute("INSERT INTO events (date, title, responsible) VALUES (%s,%s,%s)", (parse_val(d.get('date')), d.get('title'), d.get('responsible')))
    c.commit()
    cur.close()
    c.close()
    return {"status": "success"}
