...
# --- PYDANTIC MODELLE (Zwingend ganz oben definiert gegen NameErrors) ---
...
# --- DATABASE BOOTSTRAP & AUTOMATISCHE STRUKTUR-MIGRATION ---
def init_db():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS inventory (id INT AUTO_INCREMENT PRIMARY KEY, item_name VARCHAR(255), amount INT, min_amount INT, unit VARCHAR(50), location VARCHAR(100), barcode VARCHAR(100) DEFAULT '', size VARCHAR(50) DEFAULT '') ENGINE=InnoDB;")
...
# --- WEB SEITEN ROUTEN (LÖST DEN 404 FEHLER) ---
@app.get("/")
...
def api_login(data: LoginRequest, response: Response):
    ...
    raise HTTPException(status_code=401, detail="Kennwort ungültig")
...
# --- SYSTEM ACCOUNTS (LOGINS) & RECHTE-ENGINE ---
@app.get("/api/users")
...
def save_personnel_record(data: PersonnelCreateDto, request: Request):
    ...
    if data.id: cur.execute("UPDATE personnel SET name=%s, rank=%s, membership_status=%s, is_agt=%s, is_maschinist=%s, is_gf=%s, g26_3_date=%s WHERE id=%s", (data.name, data.rank, data.membership_status, data.is_agt, data.is_maschinist, data.is_gf, g26, data.id))
    else: cur.execute("INSERT INTO personnel (name, rank, membership_status, is_agt, is_maschinist, is_gf, g26_3_date) VALUES (%s,%s,%s,%s,%s,%s,%s)", (data.name, data.rank, data.membership_status, data.is_agt, data.is_maschinist, data.is_gf, g26))
    c.commit(); cur.close(); c.close(); return {"status": "success"}
...
# --- DIENSTBUCH & PROTOKOLL SYSTEM ---
@app.get("/groups")
def list_groups_legacy(request: Request):
    if not get_current_user(request): raise HTTPException(status_code=401)
...
def get_group_attendance_matrix(group_id: int, request: Request, session_id: Optional[int] = None):
    ...
    cur.execute("""
        SELECT p.id as personnel_id, p.name, p.rank, p.is_agt, p.is_maschinist,
        CASE WHEN a.is_present IS NOT NULL THEN a.is_present ELSE 0 END as is_present,
        COALESCE(a.vehicle, '') as vehicle
    ...
    cur.execute("SELECT DISTINCT description FROM sessions ORDER BY id DESC LIMIT 8")
    presets_topics = [r['description'] for r in cur.fetchall()]
    cur.execute("SELECT DISTINCT instructors FROM sessions ORDER BY id DESC LIMIT 8")
    presets_leaders = [r['instructors'] for r in cur.fetchall()]
...
# --- MATERIAL, WHITEBOARD, EVENTS CORE ---
