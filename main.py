...
# --- DATABASE ENGINE & INITIALISIERUNG ---
def init_db():
...
# --- WEB ROUNTEN OVERLAYS ---
@app.get("/")
def route_root(request: Request): 
    return FileResponse("static/dashboard.html") if get_current_user(request) else FileResponse("static/login.html")
...
def route_editor_page(request: Request): 
    return FileResponse("static/editor.html") if get_current_user(request) else FileResponse("static/login.html")
...
def api_login(data: LoginRequest, response: Response):
    ...
    if user.get('lockout_until') and user['lockout_until'] and datetime.now() < user['lockout_until']:
        cur.close(); conn.close(); raise HTTPException(status_code=423, detail="Sicherheits-Lockout aktiv. Bitte später versuchen.")
...
def get_global_weather(request: Request):
    ...
    # ==========================================================================
# DIGITALES DIENSTBUCH — ENTERPRISE ENGINE (TEIL 2 VON 2)
# ==========================================================================

# --- AUTOMATISIERTE BERICHTS- & KPI STATISTIK ENGINE ---
...
# --- SESSIONS & REAKTIVES ATTENDANCE MERGE (CRUD) ---
@app.get("/groups")
