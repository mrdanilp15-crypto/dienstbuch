...
def get_weather(r: Request):
    ...
    cur.close(); c.close()
    try:
        with urllib.request.urlopen(f"https://api.open-meteo.com/v1/forecast?latitude={s.get('station_lat','47.99')}&longitude={s.get('station_lon','10.13')}&current_weather=true", timeout=2) as res:
...
async def save_user(r: Request):
    ...
    cur = c.cursor()
    u_id = d.get('id')
    p_id = parse_val(d.get('personnel_id')); if str(p_id) == "0": p_id = None
    pw = d.get('password'); role = d.get('role', 'user'); uname = d.get('username', '').strip()
    if u_id:
        if pw and len(pw.strip()) > 0: cur.execute("UPDATE users SET role=%s, personnel_id=%s, password_hash=%s WHERE id=%s", (role, p_id, hash_password(pw), u_id))
        else: cur.execute("UPDATE users SET role=%s, personnel_id=%s WHERE id=%s", (role, p_id, u_id))
    else: cur.execute("INSERT INTO users (username, password_hash, role, personnel_id) VALUES (%s,%s,%s,%s)", (uname, hash_password(pw), role, p_id))
    c.commit(); cur.close(); c.close()
    return {"status": "success"}
...
async def save_inv(r: Request):
    d = await r.json()
    c = get_db_connection(); cur = c.cursor()
    qr_id = parse_val(d.get('qr_code_id')); if not qr_id: qr_id = f"FW-QR-{secrets.token_hex(4).upper()}"
    i_id = d.get('id')
    params = (d.get('item_name'), d.get('amount',0), d.get('min_amount',5), d.get('unit','Stück'), d.get('location','Lager'), qr_id, parse_val(d.get('last_check')), parse_val(d.get('next_check')))
...
async def inbound_webhook(req: Request):
    ...
    date_str = datetime.now().strftime('%Y-%m-%d'); desc = f"Einsatz: {keyword} - {address}"
